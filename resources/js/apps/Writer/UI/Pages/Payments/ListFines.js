...
export default function ListFines(){
    ...
    function getResult(){
        get(generateUrl(WriterApi.LIST_FINES, filters))
            .then(result => {
                setResult(result)
                setLoading(false)
            })
        ...
        <div>
            ...
            <div className="card content-card">
                ...
                <div className="card-header rounded-top d-flex align-items-center" style={{background: '#f6f9fc'}}>
                    <h2 className="heading-title mb-0 font-weight-700">Fines</h2>
                </div>
                                ...
                                (result.data ?? []).map((item, index) => {
                                    return (
                                        <tr key={item.id}>
                                            ...
                                            <td>
                                                <span className={'badge badge-' + (item.status == 'Effected' ? 'danger' : 'success')}>
                                                {item.status}
                                                </span>
                                            </td>
