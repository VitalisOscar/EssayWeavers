...
export default function ListEarnings(){
    ...
    useEffect(() => {
        setWriterData({
            ...writerData,
            pageTitle: ' All Earnings',
            headerName: 'All Earnings',
            headerIcon: 'fa fa-coins',
        })
    }, [])
    ...
    useEffect(getResult, [filters])

    function getResult(){
        get(generateUrl(WriterApi.LIST_EARNINGS, filters))
            .then(result => {
                setResult(result)
                setLoading(false)
            })
        ...
        <div>
            ...
            <div className="card content-card">
                ...
                <div className="card-header rounded-top d-flex align-items-center" style={{background: '#f6f9fc'}}>
                    <h2 className="heading-title mb-0 font-weight-700">My Earnings</h2>
                </div>
                                ...
                                (result.data ?? []).map((item, index) => {
                                    return (
                                        <tr key={item.id}>
                                            ...
                                            <td>
                                                <span className={'badge badge-' + (item.status == 'Cleared' ? 'success':(item.status == 'Pending' ? 'default':'danger'))}>
                                                {item.status}
                                                </span>
                                            </td>
