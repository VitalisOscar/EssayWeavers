...
export default function ListWriters(){
    ...
    useEffect(() => {
        setAdminData({
            ...adminData,
            pageTitle: 'My Writers',
            headerName: 'My Writers',
    ...
    useEffect(getWriters, [filters])

    function getWriters(){
        get(generateUrl(AdminApi.LIST_WRITERS, filters))
            .then(result => {
                setResult(result)
                setLoading(false)
            })
        ...
        <div>
            ...
            <div className="card content-card">
                ...
                <div className="card-header rounded-top d-flex align-items-center" style={{background: '#f6f9fc'}}>
                    <h2 className="heading-title mb-0 font-weight-700">My Writers</h2>

                    <Link to={AdminRoutes.ADD_WRITER} className="btn btn-outline-primary py-2 ml-auto">
                                ...
                                (result.data ?? []).map((item, index) => {
                                    return (
                                        <tr key={item.id}>
                                            ...
                                            <td>
                                                {item.accounts.available_formatted}
                                            </td>
                                            ...
                                            <td>
                                                <Link to={generateUrl(AdminRoutes.SINGLE_WRITER, { writer_id: item.id })}>
                                                    View
