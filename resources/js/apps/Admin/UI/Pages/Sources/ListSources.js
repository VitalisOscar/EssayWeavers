...
export default function ListSources(){
    ...
    useEffect(getSources, [filters])

    function getSources(){
        get(generateUrl(AdminApi.LIST_SOURCES, filters))
            .then(result => {
                setResult(result)
                setLoading(false)
            })
        ...
        <div>
            ...
            <div className="card content-card">
                ...
                <div className="card-header rounded-top d-flex align-items-center" style={{background: '#f6f9fc'}}>
                    <h2 className="heading-title mb-0 font-weight-700">Order Sources</h2>

                    <Link to={AdminRoutes.ADD_SOURCE} className="btn btn-outline-primary py-2 ml-auto">
                                ...
                                (result.data ?? []).map((item, index) => {
                                    return (
                                        <tr key={item.id}>
                                            ...
                                            <td>
                                                <Link to={generateUrl(AdminRoutes.SINGLE_SOURCE, {source_id: item.id})}>
                                                    View
